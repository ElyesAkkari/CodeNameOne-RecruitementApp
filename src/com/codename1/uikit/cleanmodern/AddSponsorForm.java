...
public class AddSponsorForm extends BaseForm{
    public AddSponsorForm(Resources res) {
        ...
        addSponsor.addActionListener(l -> {
            if (nom.getText().isEmpty() || type.getText().isEmpty()|| image.getText().isEmpty()) {
                ToastBar.showMessage("Fill all Text Areas", FontImage.MATERIAL_ERROR);
                //Dialog.show("Fill all Text Areas", "", "annuler","ok");
            } else {
                Sponsor e = new Sponsor(nom.getText(), type.getText(),image.getText());
                if (ServiceSponsor.getInstance().addSponsor(e)) {
                    Dialog.show("Success", "Sponsor added", new Command("OK"));
                    new SponsorForm(res).showBack();
                } else {
